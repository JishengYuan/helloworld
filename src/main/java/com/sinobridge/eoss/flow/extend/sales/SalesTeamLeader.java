 ...
 * ---------------------------------------------------------------------------------
 * 2014年7月7日 下午4:59:55          3unshine        1.0         To create
 * </p>
 *
 * @since 
 * @see     
...
@Service
@Transactional
public class SalesTeamLeader implements Assignee {
    @Autowired
        ...
        if ("admin".equals(userId)) {
            ...
            try {
                if(userId.equals("jianghb")||userId.equals("zhoushu")){
                    ...
                    userName = sysStaffService.getTeamLeader(userId);
                    list.add(userName);
                }
            } catch (Exception e) {
                e.printStackTrace();
