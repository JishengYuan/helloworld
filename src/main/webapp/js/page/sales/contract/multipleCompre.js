	        ...
	        methods :{
	        	initDocument : function(){
	        		...
	        		searchManage.doExecute('getCustomerInfo',"");
	        		 ...
	        		 //加载行业下拉框
	        		searchManage.doExecute('getIndustryName');
...
//		    							buffera.append('<a class="_customerIdt_select_a" style="" name="_customerIdt_select" id="'+result[i].id+'" href="#_select_spot">'+result[i].name+'</a>');
	            ...
	            getOrgTree:function(){
		        	...
		        	var userOrgId=$("#orgId").val();
		     		var optionsOrg = {
		     				animate : true,
		     				searchTree : true,
		     				tree_url : ctx+'/formTree/getTreeOrg?random='+userOrgId,// 顶层
		     				asyncUrl : ctx+'/formTree/getTreeOrg?random=1',// 异步
		     				search_url : ctx+'/formTree/searchTreeOrg?random='+userOrgId,// 搜索
		        ...
		        getIndustryName:function(){
		        	//行业名称
		    		var customerIndustry = $("#_customerIndustry");
		    		customerIndustry.addClass("li_form");
		    		var optionCompPosType = {
		    				writeType : 'show',
		    				showLabel : false,
		    				required:true,
		    				url : ctx+"/base/customermanage/customerIndustry/getTreeList?tmp="+Math.random(),
		    				onSelect:function(node){
		    					var id = $("#_customerIndustry").formSelect("getValue")[0];
		    					searchManage.methods.getCustomerIdtCustomer(id);
		    					$('#search_customerIndustry').val($("#_customerIndustry").formSelect("getValue")[0]);
		    				},
		    				width : "224"
		    		};
		    		customerIndustry.formSelect(optionCompPosType);
		    		searchManage.methods.getCustomerIdtCustomer("");
		        },
		      //行业客户选择下拉框
		        getCustomerIdtCustomer:function(id){
	        		...
	        		var optionCustomerIdtCustomer = {
	        				...
	        				onSelect:function(node,obj){
	        					...
	        					$('#search_customerIdtCustomer').val($("#_customerIdtCustomer").formSelect("getValue")[0]);
	        				},
	        				width : "224"
	        		};
