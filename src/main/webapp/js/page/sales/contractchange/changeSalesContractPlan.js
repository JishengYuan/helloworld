define(function(require, exports, module) {
	...
	var changeSalesContractPlan = {
			config: {
				module: 'changeSalesContractPlan',
	        ...
	        methods :{
	        	initDocument : function(){
	        		...
	        		changeSalesContractPlan.doExecute('getDataTable');
	        		changeSalesContractPlan.doExecute('getContractType');
	        		changeSalesContractPlan.doExecute('getInvoiceType');
	        		changeSalesContractPlan.doExecute('getAccountCurrency');
	        		changeSalesContractPlan.doExecute('getReceiveWay');
	        		changeSalesContractPlan.doExecute('getIndustryAndCustomerIdt');
	        		changeSalesContractPlan.doExecute('staffSelect');
	        		 //加载附件组件
	        		var typeJson = {};
	        		$.ajax({
		    			type : "GET",
	        		...
	        		changeSalesContractPlan.methods.showFileField();
	        		$("#_invoice_table tbody").find("tr").each(function(i,ele){
	        			 var invoiceTypeId=$('#invoiceType'+i).text();
	        			 changeSalesContractPlan.methods.getInvoice(invoiceTypeId,i,typeJson);
	 	        		var invoiceStatusId=$('#invoiceStatus'+i).text();
	 	        		changeSalesContractPlan.methods.getInvoiceStatus(invoiceStatusId,i);
	        		 });

		        	//详情返回按钮
		        	$("#_sino_eoss_sales_contract_detailBack").unbind('click').click(function () {
		        		changeSalesContractPlan.methods.closeWindow('detail');
		        	});
		        	changeSalesContractPlan.methods.typeSelectResult($('#contractTypeId').val());
		        	changeSalesContractPlan.methods.getContractName();
...
//			       					 totalCount+=parseInt($(elem).attr("tdPrice"));
		        	...
		        	$("#_sino_eoss_sales_contract_change_submit").unbind('click').click(function () {
		        		changeSalesContractPlan.methods.doContractChange();
		        	});
...
//	    						totalAmount+=parseInt($(elem).find("input").first().val());
			    		...
			    		if(b){
			    	     ...
			    	     $("#tableData").val(json_data);
			        		...
			        		var url = ctx+'/sales/contract/doContractOtherChange?tmp='+Math.random()+'&changeType='+$("#changeType").val();
			        		$("#_sino_eoss_sales_contract_change_submit").unbind('click');
			        		$.post(url,datas,
			    	            function(data,status){
			    	            	if(data=="success"){
			    	                	  UicDialog.Success("保存数据成功!",function(){
			    	                		  changeSalesContractPlan.methods._back();
			    	                	  });
			    	                  }else{
...
//		        		window.opener.childColseRefresh();
		        ...
		        approveToFlow : function(opts){
		        	...
		        	var options = {};
		    		options.murl = ctx+changeSalesContractPlan.config.namespace + opts.url;
		    	          ...
		    	          function(data,status){
		    	            if(status=="success"){
		    	                UicDialog.Success("审批成功!",function(){
		    	                	changeSalesContractPlan.doExecute("closeWindow","proc");
		    	                });
		    	            }else{
			...
			doExecute : function(flag, param) {
				var method =changeSalesContractPlan.methods[flag];
	...
	exports.init = function() {
		changeSalesContractPlan.doExecute('initDocument');
	}
});