		...
		<c:if test="${model.changeRemark!=null&&model.changeRemark!='' }">
       		<ul class="clearfix" >
				<li id="field_userName" class="li_form" style="width: 600px;">
					<label class="editableLabel" style="width:110px;">订单驳回原因：</label>${model.changeRemark }
					<input type="hidden" id="changeRemark" name="changeRemark"  class="row-input"  value="${model.changeRemark }"/>
				</li>
			</ul>
		</c:if>
		 <ul class="clearfix" >
			...
			<li id="field_userName" class="li_form" style="width:600px">
				...
				<input type="text" id="orderName" name="orderName"  class="row-input-oneColumn"  value="${model.orderName }" placeholder='请输入订单名称' required data-content="请输入订单名称"/>
			</li>
		</ul>
		<ul class="clearfix" >
			<li id="field_userName" class="li_form">
 ...
 -->
	...
	<input type='hidden' name='tableData' id='tableData' />
				...
				<tbody>
				  <c:if test="${product.salesContractModel.id!=null}">
					<tr style='text-algin:center' id='tr_${ status.index + 1}' name="trName" num='${ status.index + 1}' tdValue="${product.salesContractModel.id}">
						<td  id='contractName${ status.index + 1}' name="contractName" tdValue="${product.salesContractModel.contractName}" >${product.salesContractModel.contractName}</td>
						<td id='productType${ status.index + 1}' name="productType" tdValue="${product.productType }">${product.productType }</td>
						<td  id='vendorCode${ status.index + 1}' name="vendorCode" tdValue="${product.vendorCode }">${product.vendorCode }</td>
						<td  id='productNo${ status.index + 1}' name="productNo" tdValue="${product.productNo }">${product.productNo }</td>
						...
						<td  ><input  class="unitPriceStyle" num='${ status.index + 1}' type="text" style="width:90%" id='unitPrice${ status.index + 1}' name="unitPrice" value="${product.unitPrice }" /></td>
						<td ><input class="sumStyle"  num='${ status.index + 1}' type="text" style="width:90%" id='sub${ status.index + 1}' name="sub" value="${product.subTotal }"  /></td>
						...
						<td class="sino_table_label" ><div style='position: relative;' class='input-append date'><input type='text' data-format='yyyy-MM-dd' id='serviceStartTime${ status.index + 1}' style='width:55px' value='${product.serviceStartTime }'/><span class='add-on'><i data-time-icon='icon-time' data-date-icon='icon-calendar'></i></span></div></td>
 	 					<td class="sino_table_label" ><div style='position: relative;' class='input-append date'><input type='text' data-format='yyyy-MM-dd' id='serviceEndTime${ status.index + 1}' style='width:55px' value='${product.serviceEndTime }'/><span class='add-on'><i data-time-icon='icon-time' data-date-icon='icon-calendar'></i></span></div></td>
						 --%>
						<td id='productId${ status.index + 1}' style='display:none'  name="productId"  tdValue="${product.salesContractProductModel.id}"></td>
						<td id ='mark${ status.index + 1}' style='display:none' name='mark ' tdValue="${product.mark}"></td>
						<td><input type='checkbox' name='myCheckbox' value="${ status.index + 1}" /></td>
						<%-- <td class="sino_table_label"><a serial_num='${ status.index + 1}'  id='_remove_product_${ status.index + 1}' style="width:60%" class='btn btn-danger _remove_product'><i style="width:100%;font-size: 12px;">删除</i></a></td> --%>
					</tr>
					</c:if>
					<c:if test="${product.interPurchasProduct.id!=null}">
						<tr style='text-algin:center' id='tr_${ status.index + 1}' name="trName" num='${ status.index + 1}' tdValue="${product.interPurchas.id}">
							...
							<td class="sino_table_label" ><input class="quantityStyle" num='${ status.index + 1}' type="text"  min='0' style="width:90%" id='quantity${ status.index + 1}' name="quantity" value="${product.quantity }"  inputCount="${product.quantity }" /></td><input type="hidden" id="productNum${ status.index + 1}" name="productNum" value="0"/>
		...
		<li id="field_userName" class="li_form" style="width:200px;float: right;">
				<label Style="width:70px;"> 订单金额：</label><span id='tatol' style="margin-left:20px;"><fmt:formatNumber value="${model.orderAmount }" type="currency" currencySymbol="￥"/></span>元
	...
	<!--按钮组-->
    		...
    		<a id='ok_Add' role="button" class="btn btn-success">保存</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    		<a id='sp_Add' role="button" class="btn btn-success"><i class="icon-ok"></i>提交</a>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
    		<a id='no_back' role="button" class="btn "></i>取消</a>
    		</div>
     </div>
</div>
 <script language="javascript">
 var form = ${form}; 
 seajs.use('js/page/business/order/saveOrUpdate',function(order_saveOrUpdate){
		order_saveOrUpdate.init();
    });
</script>
</body>
</html>