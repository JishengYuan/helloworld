		...
		<div class="salecontent">
			...
			<div class="portlet-body form" >
				...
				<form id='_sino_eoss_sales_invoice_approveForm' class='form-horizontal' >
					...
					<table id="_sino_eoss_sales_contract_products_table" style="width:960px;" class="table  table-bordered table_order">
					  <thead>
					    <tr id="_contract_show" style="background-color: #f3f3f3;color: #888888;width:960px;">
					      ...
					      <th style="width:12%;" id="type_th">产品类型</th>
					      <th style="width:10%;" id="partner_th">产品厂商</th>
					      <th style="width:15%;" id="product_th">产品型号</th>
					       <th style="width:10%;">服务开始时间</th>
					      <th style="width:9%;">服务结束时间</th>
					      <th style="width:6%;">数量</th>
					      <th style="width:10%;">单价</th>
					      <th style="width:16%;">合计</th>
					      <th style="width:14%;">下单数量</th>
					    </tr>
						...
						<c:forEach var="salesContractProductModel" items="${model.salesContractProductModel}" varStatus="status">
							<c:if test="${salesContractProductModel.relateDeliveryProductId != null&&salesContractProductModel.relateDeliveryProductId != '' }">
								<tr>
									...
									<td name="contractProductQuantity" qtdvalue='${salesContractProductModel.quantity }'>${salesContractProductModel.quantity }</td>
									...
									<td name="_contractProduct_order" qtdvalue='${salesContractProductModel.quantity }' tdoerderproductid="${salesContractProductModel.relateDeliveryProductId }">已下单</td>
								</tr>
							</c:if>
							<c:if test="${salesContractProductModel.relateDeliveryProductId == null||salesContractProductModel.relateDeliveryProductId == '' }">
								<tr>
									...
									<td tdPrice="${salesContractProductModel.totalPrice }"> <fmt:formatNumber value="${salesContractProductModel.totalPrice }" type="currency" currencySymbol="￥"/> </td>
									<td name="_contractProduct_order" qtdvalue='${salesContractProductModel.quantity }' tdoerderproductid="${salesContractProductModel.id }">已下单</td>
								</tr>
							</c:if>
						</c:forEach>
					  </tbody>
					...
					<table id="_sino_eoss_sales_contract_readyproducts_table" style="width:960px;" class="table  table-bordered table_order">
					  <thead>
					    <tr>
					      ...
					      <th style="width:8%;">产品厂商</th>
					      <th style="width:15%;">产品型号</th>
					      <th style="width:6%;">数量</th>
					      <th style="width:8%;">关联数量</th>
					...
					<div class=" row-inline _orderModel_product" >
					   <c:forEach var="orderModel" items="${orderModel}" varStatus="status">
					        <table id="_order_${status.index }" class="table  table-bordered table_order" name="ordertable" style="width:960px;margin-top:8px;">
					           <tr>
					              <td colspan=7 style="padding:8px;font-size:14px;"><div style="float:left;width:246px;">订单编号:<a href="${ctx }/business/order/detail?id=${orderModel.id}&flat=search" target="_blank"><span style="color: #0000dd;">${orderModel.OrderCode }</span></a></div><div  style="float:left;width:400px;">订单名称:${orderModel.OrderName }</div><div style="float:left;width:150px;">到货状态:${orderModel.ArrivalStatus }</div><div  style="float:right;width:100px;">采购员:<span style="font-weight:bold;">${orderModel.Creator }</span></div></td>
					           </tr>

							    <tr style="background-color: #f3f3f3;color: #888888;">
								  <th style="width:20%;" class="_order_hidden">备货合同</th>
								  <th style="width:12%;">产品类型</th>
								  <th style="width:10%;">厂商简称</th>
								  <th style="width:13%;">产品型号</th>
								  <th style="width:10%;">下单数量</th>
								  <th style="width:13%;">单价</th>
								  <th style="width:13%;">合计</th>
								</tr>
								<c:forEach var="product"  items="${orderModel.orderProduts}" varStatus="status1">
									...
									<c:if test="${product.ContractCode==model.contractCode }">
										...
										<td class="sino_table_label _order_hidden" display:none;>
											<c:if test="${product.ContractCode!=model.contractCode }">
												<a href="${ctx }/sales/contract/contractOrderAllDetail_link?id=${product.id }" style="color:blue;" target="_blank">${product.ContractCode }</a>
											</c:if>
										</td>
										<td class="sino_table_label" >${product.typeName }</td>
										<td class="sino_table_label" >${product.partnerName }</td>
										<td class="sino_table_label" tdid="${product.salesContractProductId }" qtdvalue='${product.orderQua }'>${product.pName }</td>
										<td class="sino_table_label" name="orderQua" qtdvalue='${product.orderQua }'>${product.orderQua }</td>
			...
			<div>
			<div style="float:left;margin-left: 20px;">合同产品总数：<span id='conAllQue'></span></div>
			<div style="float:left;margin-left: 120px;">产品已下单数：<span id='ordAllQue'></span></div>
