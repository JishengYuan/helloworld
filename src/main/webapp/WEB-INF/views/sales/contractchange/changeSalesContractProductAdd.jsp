...
<!-- 遮盖层div -->
		...
		<div class="portlet-body form">
			...
			<div class=" row-inline">
				<div class='row-column' id="serviceStartDate_div">
					...
					<input type="hidden" id="serviceStartDate" value="${model.serviceStartDate }"/>
			...
			<div class=" row-inline">
				<div class='row-column' id="serviceEndDate_div">
					...
					<input type="hidden" id="serviceEndDate_input" value="${model.serviceEndDate }"/>
				</div>
			</div>
			<div class=" row-inline">
					...
					<table id="_sino_eoss_sales_contract_products_table"class="table  table-bordered table-striped">
					  ...
					  <tbody>
						<c:forEach var="salesContractProductModel" items="${model.salesContractProductModel}" varStatus="status">
							<tr style='text-algin:center' id='tr_${ status.index + 1}'><input id='_product_Type_${ status.index + 1}' type='hidden' value='${salesContractProductModel.id }'>
								...
								<td >${ status.index + 1}<input id='_contract_product_id_${ status.index + 1}' type='hidden' value='${salesContractProductModel.id }'></td>
								<td><span>${salesContractProductModel.productTypeName }</span><input id='_product_Type_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productType }'><input id='_product_Type_Name_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productTypeName }'></td>
								<td><span>${salesContractProductModel.productPartnerName }</span><input id='_product_Partner_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productPartner }'><input id='_product_Partner_Name_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productPartnerName }'></td>
								<td title="${salesContractProductModel.remark}"><span>${salesContractProductModel.productName}</span><input id='_product_No_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productNo }'><input id='_product_Name_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productName }'></td>
								<td><span>${salesContractProductModel.serviceStartDate }</span><input id='_product_startTime_${ status.index + 1}' type='hidden' value='${salesContractProductModel.serviceStartDate }'></td>
								<td><span>${salesContractProductModel.serviceEndDate }</span><input id='_product_endTime_${ status.index + 1}' type='hidden' value='${salesContractProductModel.serviceEndDate }'></td>
								<c:if test="${salesContractProductModel.productNo == '1899'}">
									<td><input id='_product_quantity_${ status.index + 1}' type='text' style='width:30px;' value='${salesContractProductModel.quantity }'></td>
								</c:if>
								<c:if test="${salesContractProductModel.productNo != '1899'}">
									<td><span>${salesContractProductModel.quantity }</span><input id='_product_quantity_${ status.index + 1}' type='hidden' value='${salesContractProductModel.quantity }'></td>
								</c:if>
								<td><span><fmt:formatNumber value="${salesContractProductModel.unitPrice }" type="currency" currencySymbol="￥"/></span><input id='_product_unitPrice_${ status.index + 1}' type='hidden' value='${salesContractProductModel.unitPrice }'></td>
								<td><span><fmt:formatNumber value="${salesContractProductModel.totalPrice }" type="currency" currencySymbol="￥"/></span><input id='_product_totalPrice_${ status.index + 1}' type='hidden' value='${salesContractProductModel.totalPrice }'><input id='_product_remark_${ status.index + 1}' type='hidden' value='${salesContractProductModel.remark }'></td>
								<td style='text-algin:center'>
									<c:if test="${salesContractProductModel.relateDeliveryProductId == null||salesContractProductModel.relateDeliveryProductId == '' }">
										...
										<input type='hidden' name='relateDeliveryProductId' /><input type='hidden' value='0'/>
									</c:if>
									<c:if test="${salesContractProductModel.relateDeliveryProductId != null&&salesContractProductModel.relateDeliveryProductId != '' }">
										<input type='hidden' value="${salesContractProductModel.relateDeliveryProductId }"/><input type='hidden' value='${salesContractProductModel.id }'/>
									</c:if>
					...
					<table id="_sino_eoss_sales_contract_maproducts_table" style="" class="table  table-bordered table-striped">
					  ...
					  <c:forEach var="salesContractProductModel" items="${model.salesContractProductModel}" varStatus="status">
							<tr style='text-algin:center' id='tr_${ status.index + 1}'><input id='_product_Type_${ status.index + 1}' type='hidden' value='${salesContractProductModel.id }'>
								...
								<td><span>${salesContractProductModel.productPartnerName }</span><input id='_product_Partner_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productPartner }'><input id='_product_Partner_Name_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productPartnerName }'></td>
								<td title="${salesContractProductModel.remark}"><span>${salesContractProductModel.productName}</span><input id='_product_No_${ status.index + 1}'  type='hidden' value='${salesContractProductModel.productNo }'><input id='_product_Name_${ status.index + 1}' type='hidden' value='${salesContractProductModel.productName }'></td>
								<td><span>${salesContractProductModel.serialNumber }</span><input id='_product_serialNumber_${ status.index + 1}' type='hidden' value='${salesContractProductModel.serialNumber }'></td>
								<td><span>${salesContractProductModel.servicePeriod }</span><input id='_product_servicePeriod_${ status.index + 1}' type='hidden' value='${salesContractProductModel.servicePeriod }'></td>
								<td><span>${salesContractProductModel.serviceStartDate }</span><input id='_product_serviceStartDates_${ status.index + 1}' type='hidden' value='${salesContractProductModel.serviceStartDate }'></td>
								<td><span>${salesContractProductModel.serviceEndDate }</span><input id='_product_serviceEndDates_${ status.index + 1}' type='hidden' value='${salesContractProductModel.serviceEndDate }'></td>
								<c:if test="${salesContractProductModel.productNo != '1899'}">
									<td><span>${salesContractProductModel.quantity }</span><input id='_product_quantity_${ status.index + 1}' type='hidden' value='${salesContractProductModel.quantity }'></td>
								</c:if>
								<c:if test="${salesContractProductModel.productNo == '1899'}">
									<td><input id='_product_quantity_${ status.index + 1}' type='text' value='${salesContractProductModel.quantity }' style="width:30px;"></td>
								</c:if>
								<td><span><fmt:formatNumber value="${salesContractProductModel.unitPrice }" type="currency" currencySymbol="￥"/></span><input id='_product_unitPrice_${ status.index + 1}' type='hidden' value='${salesContractProductModel.unitPrice }'></td>
								<td><span><fmt:formatNumber value="${salesContractProductModel.totalPrice }" type="currency" currencySymbol="￥"/></span><input id='_product_totalPrice_${ status.index + 1}' type='hidden' value='${salesContractProductModel.totalPrice }'></td>
								<td><span>${salesContractProductModel.equipmentSplace }</span><input id='_product_equipmentSplace_${ status.index + 1}' type='hidden' value='${salesContractProductModel.equipmentSplace }'><input type='hidden' value='${salesContractProductModel.id }'/></td>
							</tr>
						</c:forEach>
					  </tbody>
					</table>
	...
	<script language="javascript">
		...
		seajs.use('js/page/sales/contractchange/changeSalesContractProductAdd',function(changeSalesContractAmount){
			changeSalesContractAmount.init();
		});    
	</script>
</body>
</html>
